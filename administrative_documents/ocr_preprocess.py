...
def image_preprocess(done_dir: str, origin_data: OriginData):
...
def recognition_data_preprocess(origin: OriginDirectory, done_dir: str, origin_label_path: str, done_label_files: dict):
    with open(origin_label_path, "r") as origin_label_file:
        ...
        if origin_data.image:
            label_contents_dict = image_preprocess(done_dir, origin_data)
            for key in KEYS:
...
if __name__ == "__main__":
    ...
    start = time.time()
    with open("done/train.txt", "w") as train_label, open("done/val.txt", "w") as val_label:
        done_label_files = {
            "train":train_label,
            "val":val_label
        }
        for i, origin_label in enumerate(origin.label_list):
            ...
            if not os.path.isfile(origin_label):
                ...
                continue
            recognition_data_preprocess(origin, done.image_dir, origin_label, done_label_files)
